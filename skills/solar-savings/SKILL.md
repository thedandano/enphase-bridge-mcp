...
3. Call `refresh_tou_schedule` ONLY when the user says their rates changed or the schedule looks stale — it mutates upstream state (fetches from OpenEI), so it is not a routine read.
