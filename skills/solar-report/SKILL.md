...
- `avg_daily_produced_kwh`, `best_day`, and `worst_day` come back **null** when the range has no finished day with data (e.g. "today so far", or a week that just started). Render that line as `Daily average / best / worst: not available until a full day is complete` — never as 0 kWh.
