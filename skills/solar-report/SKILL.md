...
- `data_completeness_pct` below 100, days with `has_data` false, **or any day with `is_partial` true** mean gaps: caveat in one line. **Name the affected dates when the breakdown identifies them** — days with `has_data` false or `is_partial` true (e.g. "⚠️ Data is missing for Aug 4 and Aug 6"). If completeness is below 100 but no breakdown day is flagged (the gap is inside otherwise-recorded days, which the tool can't attribute to a date), say some readings within the period are missing without naming dates. Either way, when gaps exist don't state a trend conclusion ("production improved") — say the comparison is incomplete instead. Note `data_completeness_pct` can read 100 while today is still in progress (its denominator is windows expected *so far*) — so a range that includes today always gets the caveat, and a comparison against a completed prior period must name the partial day rather than compare as complete. The daily average already excludes unfinished/missing days — say so if the user asks why numbers look different.
